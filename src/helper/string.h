...
inline std::string NormalizeUtf8(absl::string_view text) {
...
inline bool IsBlank(absl::string_view text) {
	return ranges::all_of(text, [](char ch) {
		return absl::ascii_isspace(static_cast<unsigned char>(ch));
	});
}

inline std::string TrimAsciiWhitespaceCopy(absl::string_view text) {
	return std::string(absl::StripAsciiWhitespace(text));
}

inline std::string CanonicalizeToken(absl::string_view text) {
	std::string normalized = NormalizeUtf8(TrimAsciiWhitespaceCopy(text));
	absl::AsciiStrToLower(&normalized);
	return normalized;
}

inline std::string DefaultString(absl::string_view value, absl::string_view fallback) {
	return IsBlank(value) ? std::string(fallback) : NormalizeUtf8(value);
}

inline std::string BuildKeyValueLine(absl::string_view key, absl::string_view value) {
