...
bool RegisterCUDNNModule(v8::Isolate* isolate, v8::Local<v8::Context> context) {
    ...
    bool ok = module
                  ->Set(context,
                        ...
                        v8::Boolean::New(isolate, engine::bridge::cudnn::IsAvailable()))
                  .FromMaybe(false);

    v8::Local<v8::String> summary =
        v8::String::NewFromUtf8(isolate, engine::bridge::cudnn::Summary().c_str())
            .ToLocalChecked();
    ok = ok && module
    ...
    return context->Global()
        ->Set(context, v8::String::NewFromUtf8Literal(isolate, "CUDNN"), module)
        .FromMaybe(false);
}
