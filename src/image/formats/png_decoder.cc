...
std::shared_ptr<ImageBuffer> PngDecoder::Decode(const std::string& path,
    ...
    if (!engine::bridge::ffmpeg::DecodeVideoFrames(path, 0, 1, &frames, &err) ||
        frames.empty()) {
        flux::core::Logger().Error("PngDecoder", "Decode failed: " + err);
        return nullptr;
    }
    auto& fr = frames[0];
    desc.width = fr.width;  desc.height = fr.height;
    desc.format = PixelFormat::RGBA8;  desc.format_name = "png";
...
std::shared_ptr<ImageBuffer> PngDecoder::DecodeMemory(const uint8_t*, std::size_t,
                                                        ImageDescriptor&) {
    return nullptr;
}
